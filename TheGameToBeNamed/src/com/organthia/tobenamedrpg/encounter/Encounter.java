...
public class Encounter {
	...
	public Tile getTile(int x, int y) {
		...
		if (tiles[x + y * width] == grassTileReturn)
			return Tile.grassTile;
		if (tiles[x + y * width] == waterTileReturn)
			return Tile.waterTile;
		if (tiles[x + y * width] == lavaTileReturn)
	...
	public int[] generateTileIndex(int[] index, int tileReturn, int w, int h) {
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				if (tiles[x + y * w] == tileReturn)
					continue;
				int total = 0;
				for (int yy = 0; yy < 3; yy++) {
					for (int xx = 0; xx < 3; xx++) {
						if ((x + xx - 1) < 0 || (x + xx - 1) >= w || (y + yy - 1) < 0 || (y + yy - 1) >= h)
							continue;
